...
const AdminDashboard: React.FC = () => {
  ...
  const [partnerData, setPartnerData] = useState<
    { name: string; courseCount: number }[]
  >([]);
  ...
  const fetchTotalCourses = async () => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/partners/${partnerId}/courses`,
      {
  ...
  const fetchTotalWorkshops = async () => {
    ...
    const partnerId = localStorage.getItem("partner_id");
    const response = await fetch(
      `${API_BASE_URL}/api/partners/${partnerId}/workshops`,
      {
        headers: { Authorization: `Bearer ${access_token}` },
      }
    );
    ...
    const partnerData = await Promise.all(
      partners.map(async (partner: Partner) => {
        const coursesResponse = await fetch(
          `${API_BASE_URL}/api/partners/${partner.id}/courses`,
          {
            headers: { Authorization: `Bearer ${access_token}` },
          }
        );
    ...
    <div>
      ...
      <div className="dashboard" style={{ padding: "5px 0 0 0 " }}>
        ...
        <div
          ...
          style={{ padding: 2, backgroundColor: "#F8FAFC" }}
        >
          <ContentHeader />
          <div
