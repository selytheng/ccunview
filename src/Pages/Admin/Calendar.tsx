...
const Calendar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const todayEvent = fetchedEvents.find(
          (event: Event) =>
  ...
  const onChange = (newDate: Date) => {
    ...
    const eventOnSelectedDate = events.find((event) => {
      return (
        moment(event.start_date).format("YYYY-MM-DD") === selectedDateString
      );
    });
  ...
  const tileClassName = ({ date, view }: { date: Date; view: string }) => {
    if (view === "month") {
      ...
      return events.some(
        (event) => moment(event.start_date).format("YYYY-MM-DD") === dateString
      )
        ? "highlighted"
        : "";
    }
            ...
            >
              <p>
                ...
                <span style={{ fontFamily: "Arial" }}>
                  {moment(selectedEvent.start_date).format("MMM Do YYYY")}
                </span>
              </p>
