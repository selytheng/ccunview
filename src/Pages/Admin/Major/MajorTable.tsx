...
const MajorTable = ({ majors, onEdit, onDelete }: { majors: Major[], onEdit: (major: Major) => void, onDelete: (major: Major) => void }) => (
  <div className="major-cards" style={{ display: 'flex', flexWrap: 'wrap', gap: '22px' }}>
    {majors.map((major) => {
      const logoPath = major.logo.startsWith('storage/') 
        ? `http://localhost:8000/${major.logo}` 
        : `http://localhost:8000/storage/${major.logo}`;
      
      return (
        <Card sx={{ maxWidth: 400, marginBottom: '5px', width: 235, height: 260, display: 'flex', transition: 'transform 0.3s, box-shadow 0.3s', '&:hover': { transform: 'scale(1.01)', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', }, }} key={major.id} >
          <CardActionArea>
            <CardMedia component="img" image={logoPath} alt={major.name}
               style={{  padding: 5, width: '100%', height: '200px', objectFit: 'cover', borderBottom: '1px solid #c3baba' }}
            />
            <CardContent style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div className="card-actions" style={{ display: 'flex', marginTop: '-10px', marginLeft: 20 }}>
