...
const EventComponent: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/events'); // Corrected the URL
                                                                ...
                                                                <Grid item xs={12} sm={6} md={3} key={event.id}>
                                                                    ...
                                                                    >
                                                                        {event.image && (
                                                                            <CardMedia
                                                                                sx={{ height: 170 }}
                                                                                image={`http://localhost:8000/${event.image}`} // Ensure this URL is correct
                                                                                title={event.title} // Use title for accessibility
                                                                            />
                                                                        )}
                                                                        <CardContent>
                                                                            <Typography gutterBottom variant="h6" component="div">
                                                                                <Link to={`/event/${event.id}`} // Update the link to point to events
                                                                                      style={{
                                                                                          textDecoration: 'none',
                                                                                          color: '#000'
                                                                                      }}>
                                                                                    <div className="event-title" // Change class name to reflect events
                                                                                         style={{
                                                                                             fontWeight: 'bold',
                                                                                             fontSize: '1.2rem'
                                                                                         }}>
                                                                                        {event.title} {/* Ensure title is displayed */}
                                                                                    </div>
                                                                                </Link>
                                                                            </Typography>
                                                                            <Typography
                                                                            ...
                                                                            >
                                                                                <div className="event-description">{event.description}</div> {/* Change class name to reflect events */}
                                                                            </Typography>
                                                                        </CardContent>
                                                                    </Card>
                                                                </Grid>
